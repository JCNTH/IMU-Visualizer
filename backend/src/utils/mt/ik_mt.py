...
def get_imu_orientation_mt(imu_data_mt, f_type, fs = constant_mt.MT_SAMPLING_RATE, dim = '9D', params = None, get_time = False):
    ''' Get orientation from all sensors
...
def quat_to_euler(quat, to_deg = True):
    ...
    '''
    # x_angle = math.atan2(2*quat[2]*quat[3] + 2*quat[0]*quat[1],
    #                      2*quat[0]**2 + 2*quat[3]**2 - 1)
    # y_angle = math.asin(np.round(2*quat[1]*quat[3] - 2*quat[0]*quat[2], 4))
    # z_angle = math.atan2(2*quat[1]*quat[2] + 2*quat[0]*quat[3],
    ...
    angle 	= r.as_euler('zxy', degrees = True)
    # angle 	= r.as_euler('ZXY', degrees = True)
    angle_x	= angle[1]
    angle_y	= angle[2]
    angle_z	= angle[0]

    angles_3d = [angle_x, angle_y, angle_z]
...
def get_pelvis_angles(pelvis_ori, s2s_pelvis):
    ''' Get pelvis angles from the provided orientation

    Args:
        + pelvis_ori (quat_array, len = N): orientation of pelvis sensor
        + s2s_pelvis (np.array, shape = 3x3): segment-to-sensor of pelvis

    Returns:
        + pelvis_angles (np.array, shape = N x 3): 3-dof angles of pelvis
    '''

    N = pelvis_ori.shape[0]
    pelvis_angles = []

    s2s_pelvis = quaternion.from_rotation_matrix(s2s_pelvis)

    segment_pelvis = [pelvis_ori[i]*s2s_pelvis.conjugate() for i in range(N)]
    # segment_pelvis = [pelvis_ori[i] for i in range(N)]
    
    pelvis_rot = [segment_pelvis[i] for i in range(N)]
    pelvis_rot = quaternion.as_float_array(pelvis_rot)
    pelvis_angles = [quat_to_euler(pelvis) for pelvis in pelvis_rot]
    pelvis_angles = np.array(pelvis_angles)
    
    assert pelvis_angles.shape == (N, 3), 'Incorrect data shape'
...
def get_all_ja_mt(seg2sens, orientation_mt, c_flag = True):
    ...
    mt_ja['pelvis_list'] = constant_common.JA_SIGN['pelvis_list']*pelvis_angles[:, 1]
    mt_ja['pelvis_rot']  = constant_common.JA_SIGN['pelvis_rot']*pelvis_angles[:, 0]
    ...
    # mt_ja['ankle_angle_l']    = constant_common.JA_SIGN['ankle_angle_l']*temp_ankle_l[:, 2]
    mt_ja['hip_adduction_l']   = constant_common.JA_SIGN['hip_adduction_l']*temp_hip_l[:, 0]
    mt_ja['hip_rotation_l']    = -1*constant_common.JA_SIGN['hip_rotation_l']*temp_hip_l[:, 1]
    mt_ja['hip_flexion_l']     = constant_common.JA_SIGN['hip_flexion_l']*temp_hip_l[:, 2]
    ...
    mt_ja['ankle_rotation_l']  = -1*constant_common.JA_SIGN['ankle_rotation_l']*temp_ankle_l[:, 1]
    ...
    # mt_ja['ankle_angle_r']    = constant_common.JA_SIGN['ankle_angle_r']*temp_ankle_r[:, 2]
    mt_ja['hip_adduction_r']   = constant_common.JA_SIGN['hip_adduction_r']*temp_hip_r[:, 0]
    mt_ja['hip_rotation_r']    = -1*constant_common.JA_SIGN['hip_rotation_r']*temp_hip_r[:, 1]
    mt_ja['hip_flexion_r']     = constant_common.JA_SIGN['hip_flexion_r']*temp_hip_r[:, 2]
    ...
    mt_ja['ankle_rotation_r']  = -1*constant_common.JA_SIGN['ankle_rotation_r']*temp_ankle_r[:, 1]
...
def get_static_offset_mt(static_ja_mt):
    ''' Get static offset from the static joint angles

    Args:
        + static_ja_mt (dict of np.array): static joint angles
    
    Returns:
        + static_offset_mt (dict of float): static offset
    '''
    static_offset_mt = {}

    for joint in static_ja_mt.keys():
        static_offset_mt[joint] = np.mean(static_ja_mt[joint])
